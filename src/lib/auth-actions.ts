...
export async function login(formData: FormData) {
  ...
  try {
    ...
    let role;
    if (employeeDomains.includes(emailDomain)) {
      const { data, error: employeeError } = await supabase.from("employees").select("role").eq("contact_info", email).single();
      if (employeeError) throw employeeError;
      role = data?.role;
    } else {
      const { data, error: customerError } = await supabase.from("customers").select("role").eq("email", email).single();
      if (customerError) throw customerError;
      role = data?.role;
    }
...
export async function signup(data: { firstName: string, lastName: string, email: string, password: string }) {
  ...
  if (user) {
    if (domainData) {
      ...
      if (fetchError && fetchError.code !== 'PGRST116') {
        console.error("Error fetching existing employee:", fetchError.message);
        return { error: "An error occurred while creating your account. Please contact support." };
      }
      ...
      if (existingEmployee) {
        ...
        if (updateError) {
          ...
          return { error: "An error occurred while updating your account. Please contact support." };
        }
      } else {
        // Insert new employee
